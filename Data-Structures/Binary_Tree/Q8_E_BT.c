...
int hasGreatGrandchild(BTNode *node)
{
    ...
    left_node = hasGreatGrandchild(node -> left);
    right_node = hasGreatGrandchild(node -> right);
    if(left_node >= 3 || right_node >= 3)
    printf("%d ", node -> item);

    return (left_node > right_node ? left_node : right_node) + 1;
}
